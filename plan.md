...
| 2026-02-13 | Backlog Closure | Enforced strict auth-claim scope model end-to-end by removing hybrid toggle UI, making login/session checks always claim-required, hardening Firestore rules to fail closed without claims, and updating runbook/UAT guidance accordingly | `src/components/SupervisorDashboard.jsx`, `src/components/PinLogin.jsx`, `src/App.jsx`, `src/services/authPolicyService.js`, `firestore.rules`, `scripts/provisionAuthClaims.js`, `docs/CUTOVER_RUNBOOK.md`, `docs/UAT_WALKTHROUGH_PHASE9.md`, `plan.md` | `npm run smoke:phase9:full` |
