...
Node.js 22 runtime parity is verified locally: the readiness verifier reported `runtimeParity: true`, all 52 focused security contracts passed, and the Functions emulator loaded every dormant callable and passed 10 tests under Node.js 22.23.2. Already-large EOC/issue mutations still require their protected server transaction at strict cutover. Production release approval and all production configuration, secret, deployment, activation, and canary actions remain separate and unperformed.
