...
- After deployment, the Firestore compiler identified helpers made unreachable by compatibility retirement. PR #23 removed the first 16 with the strict suite still green; its successful rules-only release exposed six transitive helpers used only by that deleted code. PR #24 removed those final definitions. The final static scan found zero unreferenced rule helpers, the strict Firestore suite again passed 17 current tests with 23 intentionally retired cases skipped and 0 failures, and the final Firestore rules-only release compiled and deployed without warnings.
...
- Local verification: security foundation 101 passed, including four final-retirement contract tests; Firestore/Auth/Functions security contracts 44 passed (including the new all-active previously-non-enrolled login contract), Firestore rules 40 passed, Storage rules 4 passed, and the aligned non-enrolled compatibility login/reload browser gate 1 passed. The retirement inventory correctly remains pending on the seven known compatibility groups and passed all secure-behavior preservation checks. ESLint passed; the production build passed with 1,913 modules; JavaScript syntax, readiness, and `git diff --check` passed. The emulator products were green before the known Firebase CLI updater-cache shutdown error. The Functions emulator used host Node 24 because local Node 22 remains unavailable; production Functions still declare and run Node 22, so no new local Node 22 parity is claimed. The temporary emulator-only dummy secret was removed immediately.
- With Mark's explicit authorization, commit `3ef5368` was pushed, PR #19 merged into case-sensitive `Main` as `e54322b`, and a guarded `v3-enabled` bundle was deployed to Firebase Hosting only. The deployment uploaded 17 files and completed successfully; Functions, Firestore rules, Storage rules, Auth providers, protected configuration, accounts, sessions, PINs, and production data were not changed.
- Direct release verification passed. Live HTML returned 200 and referenced `index-3Gd50sTS.js`; that exact main asset loaded `App-BsqUAcaI.js`, which returned 200 and contained the cutover message/auth-policy listener. The already signed-in Lone Mountain Home page reloaded with normal transport, issue, EOC, debrief, and quick-action controls and zero captured browser-console errors.
...
2. **Complete:** merge and Hosting-deploy the compatibility-session cutover guard plus guarded cutover/rollback tooling.
