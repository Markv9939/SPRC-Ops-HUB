...
Current `Main` reference: privacy-safe v13 evidence PR #18 merged as `c962266`; Hosting remains the verified v13 release from `30c5665`
...
| **Prepared locally** | A strict-cutover guard now monitors the live auth policy and automatically returns any remaining compatibility session to the familiar six-digit PIN screen when strict authorization is eventually enabled. A privacy-safe read-only closeout audit verifies all active profiles, credentials, stable identities, secure-login coverage, workflows, App Check observation, and strict-auth state. A separate checksum-backed cutover command now previews, applies, and rolls back the all-active and strict stages with exact confirmations and immutable audit evidence. A final machine-checked retirement contract inventories every remaining browser, server, session, Firestore, and Storage compatibility path and also prevents cleanup from removing the six-digit UX, protected server login/account actions, or 84-hour session contract. These source changes are tested locally but not yet merged or deployed. |
...
12. **Prepared locally; release pending:** Deploy the compatibility-session cutover guard, privacy-safe readiness audit, and checksum-backed cutover/rollback command. The guard returns a legacy browser session to the PIN screen when strict authorization turns on instead of leaving staff on a broken permission-error screen. The command changes only the exact target setting, refuses stale previews, requires exact confirmation, creates immutable audit evidence, and restores the verified configuration; all-active rollback also closes sessions outside the prior allowlist and revokes their refresh tokens.
