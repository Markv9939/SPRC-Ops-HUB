...
| **In progress** | Security-foundation Phases 1–8 are implemented locally behind disabled versioned gates: server PIN/custom-token identity, persistent 84-hour device sessions, revocation/account actions, owner-bound offline replay, named workflow rules/Functions, two-device conflict protection, App Check monitoring readiness, and canary/rollback checks. The normal build and current production PIN login remain unchanged because nothing is enabled, configured, deployed, pushed, or merged. |
...
9. **Retire compatibility mode — gated:** Node.js 22 parity is locally verified. Retirement is not authorized and cannot begin until every workflow canary, rollback practice, and separate production approval pass.
...
- Security foundation: Phases 1–8 and Node.js 22 runtime parity are verified locally behind disabled boundaries. Production configuration, canary selection/execution, deployment, activation, and compatibility retirement remain unperformed and unauthorized.
...
| Security foundation | Phases 1–8 locally implemented and dormant | Server/client identity, protected account/session actions, offline replay ownership, named workflow/rule/Storage boundaries, protected transport conflict prevention, monitoring-only App Check hooks, canary/rollback readiness, and Node.js 22.23.2 runtime parity are locally tested and saved in one isolated-branch checkpoint commit. The normal build keeps current PIN login; production configuration/data/providers/enforcement are unchanged. No deploy, push, merge, secret configuration, or activation occurred. |
